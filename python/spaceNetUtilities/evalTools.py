...
def iou(test_poly, truth_polys, truth_index=[]):
    ...
    if truth_index:
        ...
        for fid in fidlist:
            ...
            if intersection_result.GetGeometryName().endswith('POLYGON'):
        ...
        for idx, truth_poly in enumerate(truth_polys):
            ...
            if intersection_result.GetGeometryName() == 'POLYGON' or intersection_result.GetGeometryName() == 'MULTIPOLYGON':
                intersection_area = intersection_result.GetArea()
...
def score(test_polys, truth_polys, threshold=0.5, truth_index=[],
    ...
    for test_poly in test_polys:
        if truth_polys:
            ...
            if maxiou >= threshold:
                true_pos_count += 1
                truth_index.delete(fidlist[np.argmax(iou_list)],
                                   truth_polys[fidlist[np.argmax(iou_list)]].GetEnvelope())
